...
const static int CLOSE_FRAME_SENT_BIT = BIT1;
const static int REQUESTED_STOP_BIT = BIT2;
...
static void esp_websocket_client_task(void *pv)
{
 ...
 if (client->transport == NULL) {
 ESP_LOGE(TAG, "There are no transports");
 client->run = false;
 break;
 }
...
static esp_err_t esp_websocket_client_close_with_optional_body(esp_websocket_client_handle_t client, bool send_body, int code, const char *data, int len, TickType_t timeout)
{
 ...
 int close_ret;
 if (send_body) {
 close_ret = esp_websocket_client_send_close(client, code, data, len + 2, portMAX_DELAY);
 } else {
